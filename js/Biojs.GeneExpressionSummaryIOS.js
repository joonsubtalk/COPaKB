...
{	
	...
	_drawTemplate: function(){
		...
		this._title = jQuery("<h5></h5>").appendTo(this._container);
		
		this._table = jQuery("<div class='row-fluid'></div>").appendTo(this._container);
		this._row = jQuery("<div class='row-fluid'></div>").appendTo(this._table);
		this._leftColumn = jQuery("<div class='span4'></div>").appendTo(this._row);
		this._rightColumn = jQuery("<div class='span8'></div>").appendTo(this._row);
	...
	_setFeatures: function ( f ) {
		...
		for ( i = 0; i < f.length; i++ ) {
			...
			if ( Biojs.GeneExpressionSummary.TYPE_DESCRIPTION == feature.TYPE.id ) {
			    this._title.text(feature.NOTE[0].text + " (Gene Expression Atlas)");
				...
				var caption = "<p style='italics'>"+ feature.LINK.text;
				...
				this._footerContainer.append(footer);
                         ...
                         else if (feature.label != 'phenotype' && feature.label != 'developmental stage' && feature.label != 'Infection' && feature.label != 'Provenance') {
