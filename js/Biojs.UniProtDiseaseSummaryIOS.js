...
Biojs.UniProtDiseaseSummary = Biojs.extend (
	...
	_drawHtmlTemplate: function (){
		...
		var _UDS_container = jQuery('<div style="width:' + this.opt.width + '" id="'+this._component_wrapper_id+'"></div>').appendTo(_container);
		jQuery("<div id='UDS_title'></div>").appendTo(_UDS_container);
		jQuery("<div id='UDS_info'></div>").appendTo(_UDS_container);
		jQuery("<div id='UDS_warnings'></div>").appendTo(_UDS_container);
	},
	/* 
	...
	_drawComponentTitle: function (){
		if(this.opt.componentTitle){
			var _selector = "#UDS_title";
			var _container = jQuery(_selector);
			var _tableTitle = jQuery("<div class='UDS_table_title'><h5>Involvement in disease (OMIM)</h5></div>").appendTo(_container);
		}
	},
	/* 
		...
		for(i=0; i<this._comments.length; i++){
			...
			if (isVisible) {
				...
				if (omimAccs.length > 0) {
					...
					omimAccs = "<span class='UDS_omim_title'>OMIM Reference Number:</span><div class='UDS_accs'>" + omimAccs + "</div>";
				...
				if (pubmedAccs.length > 0) {
					...
					pubmedAccs = "<span class='UDS_pubmed_title'>Pubmed Reference Number:</span><div class='UDS_accs'>" + pubmedAccs + "</div>";
